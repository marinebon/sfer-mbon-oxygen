...
1. **Background pass** — regional correlation length (`0.30°`), moderate data fidelity (`epsilon2 = 1.5`). Carries the broad gradient across the gaps between transect lines.
2. **Fine pass** — short correlation length (`0.07°`), moderate data fidelity (`epsilon2 = 1.0`). Adds a gentle local residual correction on top of the background.
...
Correlation lengths near a single station spacing produce **localized discs** around each cast: the field matches observations out to roughly the background length, then drops sharply back toward the depth-slice mean. Measured nearest-neighbour station spacing is ~0.06–0.09° (median) across cruises, with 0.16–0.39° gaps on transect ends, so a background length of `0.10°` — about one spacing — cannot bridge adjacent casts: each one sits in its own ~10 km bubble. The finer output grid (`0.005°`) sharpened the disc edges rather than causing them.

Parameters were retuned so the **background pass is genuinely regional** — several station spacings — and the fine pass is a small correction rather than a hard high-fidelity stamp:

| Parameter | Previous | Current | Why |
|-----------|----------|---------|-----|
| `LEN_HORIZ_BG_DEG` | 0.10° | **0.30°** | ~3–5 median station spacings; background carries a real trend across inter-transect gaps instead of decaying to the slice mean |
| `EPSILON2_BG` | 3.0 | **1.5** | With a long length, keep enough data fidelity that the regional trend still tracks the casts |
| `LEN_HORIZ_DEG` | 0.045° | **0.07°** | Fine residual bridges to neighbouring casts (~one station spacing) instead of dying between them |
| `EPSILON2` | 0.5 | **1.0** | Softer local pull; reduces the abrupt disc edge from the fine pass |
| `CLAMP_RADIUS_DEG` | 0.10° (`= LEN_HORIZ_BG_DEG`) | **0.10°** (fixed) | Decoupled from the background length so a regional-scale background does not drag the clamp radius across a third of a degree |

The earlier tuning was validated on WS0612, which has the tightest station spacing (median 0.056°) of the batch; `0.10°` looked like ~2 spacings there but is ~1 spacing or less on sparser cruises such as WS16207.
...
where `local_min` and `local_max` are the minimum and maximum dissolved oxygen among all observations **in that depth slice** within **`0.10°`** of the cell center (`CLAMP_RADIUS_DEG`, roughly one station spacing). Cells with no observations within that radius are left unchanged.
...
| `LEN_HORIZ_DEG` | 0.07° | Fine DIVAnd correlation length (~1 station spacing) |
| `EPSILON2` | 1.0 | Fine data fidelity (signal-to-noise) |
| `LEN_HORIZ_BG_DEG` | 0.30° | Background correlation length (~several station spacings) |
| `EPSILON2_BG` | 1.5 | Background data fidelity |
| `CLAMP_RADIUS_DEG` | 0.10° | Local observation search radius for post-interpolation clamping (fixed, ~1 station spacing) |
