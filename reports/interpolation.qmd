...
1. **Background pass** — long correlation length (`0.08°`), low smoothness penalty (`epsilon2 = 1.0`). Captures broad regional gradients.
2. **Fine pass** — short correlation length (`0.035°`), higher smoothness penalty (`epsilon2 = 0.01`). Fits local residuals on top of the background.
...
|---------|---------|-------|
| Horizontal target resolution | `0.01°` (~1 km) | Used to size the lon/lat axes |
| Vertical target resolution | `4.0 m` | Used to size the depth axis |
| Minimum axis length | 30 points | Floor on grid extent |
| Maximum axis length | 150 points | Ceiling per axis |
| Maximum total grid points | 120 000 | If exceeded, all three axes are scaled down equally |
...
Depth slice tolerance (observations assigned to a level) is `max(2 m, half the native depth spacing)`.
...
1. **`extend_sea_mask_along_water!`** — walks up to 4 grid steps from each observation through cells with valid sub-tidal elevation.
2. **`bridge_unconnected_observations!`** — for observations not in the main sea component, finds a shortest path through high-resolution bathymetry (up to 40 steps) and marks those cells as sea. Paths that cross land are rejected. Observations more than 12 grid cells from the main field are skipped.
...
where `local_min` and `local_max` are the minimum and maximum dissolved oxygen among all observations **in that depth slice** within **`0.08°`** of the cell center (`CLAMP_RADIUS_DEG`, equal to the background correlation length). Cells with no observations within that radius are left unchanged.
...
|----------|-------|---------|
| `HORIZ_RES_DEG` | 0.01° | Target horizontal grid spacing |
| `DEPTH_RES_M` | 4.0 m | Target vertical grid spacing |
| `LEN_HORIZ_DEG` | 0.035° | Fine DIVAnd correlation length |
| `EPSILON2` | 0.01 | Fine smoothness penalty |
| `LEN_HORIZ_BG_DEG` | 0.08° | Background correlation length |
| `EPSILON2_BG` | 1.0 | Background smoothness penalty |
| `CLAMP_RADIUS_DEG` | 0.08° | Local observation search radius for post-interpolation clamping |
| `BBOX_PADDING_DEG` | 0.02° | Grid padding beyond observations |
| `LAND_ELEVATION_M` | −0.2 m | Land/sea threshold |
| `OBS_SEA_MAX_STEPS` | 4 | Sea-mask extension from casts |
| `OBS_BRIDGE_MAX_STEPS` | 40 | Max path length for observation bridging |
| `OBS_BRIDGE_MAX_GRID_DIST` | 12 | Max grid cells for bridging to main field |
